...
int dnsConnection (const char *hostname, struct DnsAnswer *dnsAnswer)
{
    if (hostname == NULL)
        return -1;
        ...
        switch (dnsAnswerBody->qType)
        {
            ...
            case DNS_HOST_T:
                ...
                if (dnsAnswerBody->dataLen == 4)
                {
                    ...
                    inet_ntop(AF_INET, dnsAnswerBody->data, dnsAnswerBody->data,
                        sizeof(struct sockaddr));
                }
        }
...
{
    for (struct DnsAnswerBody *i = dnsAnswer->body; i != NULL; ++i)
    {
        free(i->name);
        free(i->data);
        free(i);
...
int main ()
{
    ...
    dnsConnection("www.baidu.com", dnsAnswer);
